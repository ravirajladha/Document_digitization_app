...
use App\Models\{Receiver,Master_doc_data,Advocate,Master_doc_type,Category};
   ...
   'advocate_documents.updated_at as updated_at',
    ...
    public function childDocumentReports(Request $request)
    {
        $filters = $request->only([
            ...
            'end_date', 'area_range_start', 'area_range_end', 'area_unit', 'advocate_name','case_result',
    ...
    public function exportFilteredDocuments(Request $request)
    {
        ...
        // Loop through documents to fetch child table data based on document_type_name
        foreach ($documents as &$document) {
            // Query the child table using document_type_name and doc_id (assuming you might have multiple rows)
            ...
            if ($childData->isNotEmpty()) {
                $document['child_data'] = $childData->toArray();
            } else {
                $document['child_data'] = []; // Default to an empty array if no child data found
            }
        }
